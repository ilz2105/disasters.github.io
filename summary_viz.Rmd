...
## Fire, Earthquakes, Hurricanes, & Tornadoes by Region, 1953-2018

```{r, panel, echo = FALSE, warning= FALSE, message=FALSE}
panel = 
final_data %>% 
  group_by( disaster, region)%>% 
  dplyr::select(region, disaster, year, n_total)%>% 
...
ggplotly(panel) %>% 
  layout(title = "Number of Specific Disasters in Each Region",
...
final_data %>% 
  ...
  layout(title = "Total Precipitation by Region, from 1953 to 2018",
         xaxis = list(title = "Total Precip (in.)",
                      zeroline = TRUE),
...
This plot shows the breakdown of the total precipitation amount by state for each year, and was split up into regions, with the total precipitation summed up for each region. The southeast had overall the most inches in precipitation, while the west had the least amount of precipitation. 

## Plot showing Total Number of Disasters by Region from 1953-2018, 
...
```{r, echo = FALSE, warning= FALSE, message=FALSE}
 final_data %>% 
  ...
  layout(title = "Total Disasters by Region, from 1953- 2018",
         xaxis = list(title = "Total Number of Disastesr",
                      zeroline = TRUE),
         yaxis = list(title = "Year",
                      zeroline = TRUE))
```

This plot shows the number of disasters in each region from 1953 to 2018. We can see that the number of disasters has been on the rise and the total in the 2000s is larger compared to 1970-1980. In 2005, the southeast had 763 disasters which is the spike in the graph.Overall, the northeast and western regions had the fewest number of disasters while the midwest and southeast had the most. 
...
This bar graph shows the total number of different disasters for each region in the US, from 1953-2018. The individual number of disasters for each state are shown along the bars. We observed that the southeast had the most hurricanes, while the midwest had a greater frequency of floods. 
