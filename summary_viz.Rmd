...
This is a panel showing the total number of earthquakes, fires, hurricanes, and tornadoes from 1953-2018 for each region. The disasters are shown in different colors.We see that the Western region had no hurricanes while the other three regions did not have any earthquakes.  
...
This plot shows the breakdown of the total precipitation amount by state for each year, and is split up into regions and the total precipitation is summed up for each region. The southeast had overall the most inches in precipitation, while the west had the least amount of precipitation. 
...
This bar graph shows the total number of disasters for each region in the US, from 1953-2018. By hovering over the bars, you can see the individual number of disasters for each state.We can see that the southeast had the most hurricanes, while the midwest had a great frequency of floods. 
...
This box plot shows the median, maximum, and minimum values for the count of total disasters, and it also shows the count of disaster type for each state. For example, for `hurricane`, the median is 18, maximum is 763, minimum is 1, and Louisiana had 161 hurricanes from 1953 to 2018. 
